...
output_dir = r"/Users/raffaele/PycharmProjects/Bachelorthesis/Gain_Planes/HoN"
...
# write to text file:
# Output file name
output_file_mgo = os.path.join(output_dir, 'HoN_planes_gain.pdf')
...
plt.yscale('log')
plt.savefig(output_file_mgo)
plt.grid(linewidth = 0.5, color = 'grey')


for i in range(len(wavelengths)):
    if i == 0:
        plt.text(wavelengths[i] - 0.03, gains_planes[i] + 0.5, lattice_planes[i], fontsize=10)
    else:
        plt.text(wavelengths[i]+0.01, gains_planes[i]+0.5, lattice_planes[i], fontsize = 10)

plt.title('Gain estimate for different lattice planes of HoN')
plt.savefig('HoN/HoN_planes_gain.pdf')
plt.show()
...
#storing as csv
df.to_csv('HoN/HoN_planes_gain.csv')
