...
output_dir = r"/Users/raffaele/PycharmProjects/Bachelorthesis/Gain_Planes/Ho2O3"
...
print("Alle Keys:", list(materials_data.keys()))

# same procedure as before but for a different material.
material_name = 'Ho2O3'
a = 10.58 #Anstrom
lattice_planes = np.array([[1, 1, 0], [2, 0, 0], [2, 0, 0], [2, 1,1], [2,2,0], [2, 2, 2],[3, 1, 0],[3, 2, 1], [4,0,0], [4, 2, 0], [5,1, 0]])
...
#    materials_data[material_name]['wavelength'] = wavelengths[i]
 ...
 #   print(r"Gain  = ", gains_planes[i], " Plane = ", lattice_planes[i])
...
# write to text file:
# Output file name
output_file_mgo = os.path.join(output_dir, 'Ho2O3_planes_gain.txt')
...
plt.title('Gain estimate for different lattice planes of Ho2O3')
# -------------------Uncomment to save ----------------
plt.savefig('Ho2O3/Ho2O3_planes_gain.pdf')
#plt.show()
...
n_111 = functions.find_max_n(materials_data, material_name)

print('n_200 = ',n_111)


kappa = np.pi * n_111 / a*1e-1
print('coupling_constant = ',kappa)
...
#storing as csv
df.to_csv('Ho2O3/Ho2O3_planes_gain.csv')
