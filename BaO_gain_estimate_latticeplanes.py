...
import functions
from MgO_gain_estimate_latticeplanes import lattice_planes
...
# write to text file:
# Output file name
output_file_bao = os.path.join(output_dir, 'BaO_planes_gain.txt')

# Write to the file
with open(output_file_bao, 'w') as f:
    # Write the data
...
plt.title('Gain estimate for different lattice planes of BaO')
plt.savefig('BaO/BaO_planes_gain.pdf')
plt.show()
